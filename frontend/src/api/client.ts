...
export const api = {
  ...
  deleteNode: (uuid: string) =>
    request<void>(`/nodes/${uuid}`, { method: "DELETE" }),
  metrics: (
    node: string,
    names: string[],
    fromTs: number,
  ...
  createBookmark: (body: {
    ...
    request<BookmarkInfo>("/bookmarks", {
      method: "POST",
      body: JSON.stringify(body),
    }),
  patchBookmark: (
    ...
    request<BookmarkInfo>(`/bookmarks/${id}`, {
      method: "PATCH",
      body: JSON.stringify(body),
    }),
  deleteBookmark: (id: number) =>
    request<void>(`/bookmarks/${id}`, { method: "DELETE" }),
  orderBookmarks: (ids: number[]) =>
    request<void>("/bookmarks/order", {
      method: "PUT",
      body: JSON.stringify({ ids }),
    }),
  resolveIcons: (names: string[]) =>
    request<Record<string, string | null>>(
      `/icons?names=${encodeURIComponent(names.join(","))}`,
    ),
  putServiceMeta: (
    nodeUuid: string,
